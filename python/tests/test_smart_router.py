...
--------------------
Tests for the SmartRouter (llama.cpp only).
...
def make_provider(name, healthy=True, latency=100.0, cost=0.002, errors=0, requests=0):
    p = Provider(
        name=name,
        ping_url=f"http://localhost:8080/v1/models",
...
def test_score_always_configured():
    # llama.cpp provider is always considered configured (no API key needed)
    p = make_provider("llamacpp")
    assert p.is_configured is True


def test_score_latency_strategy_prefers_faster():
    fast = make_provider("fast", latency=50.0)
    slow = make_provider("slow", latency=500.0)
    assert fast.score("latency") < slow.score("latency")


def test_score_strategy_param_accepted():
    # New score() ignores strategy (only latency+error_rate); just ensure no crash
    p = make_provider("test", latency=200.0)
    assert p.score("cost") == p.score("latency") == p.score("balanced")
...
def test_get_model_returns_provider_model():
    # llama.cpp only has one model; always returns provider.model
    p = make_provider("llamacpp")
    r = make_router()
...
def test_get_model_large_request_also_returns_provider_model():
    p = make_provider("llamacpp")
    r = make_router()
...
def test_get_model_small_request_returns_provider_model():
    p = make_provider("llamacpp")
    r = make_router()
...
async def test_route_returns_best_provider():
    # route() always picks first healthy provider (llama.cpp only)
    p1 = make_provider("a", latency=50.0)
    p2 = make_provider("b", latency=200.0)
    r = make_router(providers=[p1, p2])
    result = await r.route([{"role": "user", "content": "Hi"}], "claude-haiku")
    assert result["provider"] in ("a", "b")


@pytest.mark.asyncio
async def test_route_returns_model_from_provider():
    p = make_provider("llamacpp")
    r = make_router(providers=[p])
    result = await r.route([{"role": "user", "content": "x" * 3001}], "claude-haiku")
