...
import { getOllamaChatBaseUrl, getLlamaCppChatBaseUrl } from './providerDiscovery.ts'
...
export type ProviderProfile = 'openai' | 'ollama' | 'codex' | 'gemini' | 'atomic-chat' | 'llamacpp'
...
export function buildLlamaCppProfileEnv(
  options: {
    baseUrl?: string | null
    model?: string | null
  } = {},
  ...
  return {
    OPENAI_BASE_URL: getLlamaCppChatBaseUrl(options.baseUrl ?? undefined),
    OPENAI_MODEL: options.model ?? 'local-model',
  }
}
...
export function selectAutoProfile(
  ...
  // llamacpp is the default local provider; fall back to openai only if needed
  return recommendedOllamaModel ? 'llamacpp' : 'openai'
}
...
export async function buildLaunchEnv(options: {
  ...
  if (options.profile === 'llamacpp') {
    // llama-server exposes an OpenAI-compatible API — no key needed
    const llamacppBaseUrl =
      process.env.LLAMACPP_BASE_URL || 'http://localhost:8080'
    env.OPENAI_BASE_URL = persistedEnv.OPENAI_BASE_URL || `${llamacppBaseUrl}/v1`
    env.OPENAI_MODEL =
      persistedEnv.OPENAI_MODEL || process.env.OPENAI_MODEL || 'local-model'
