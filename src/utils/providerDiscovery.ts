...
// ── llama.cpp (llama-server) discovery ───────────────────────────────────────

export const DEFAULT_LLAMACPP_BASE_URL = 'http://localhost:8080'

export function getLlamaCppApiBaseUrl(baseUrl?: string): string {
  const raw = baseUrl || process.env.LLAMACPP_BASE_URL || DEFAULT_LLAMACPP_BASE_URL
  const parsed = new URL(raw)
  const pathname = trimTrailingSlash(parsed.pathname)
  // Strip trailing /v1 if present so we get the bare base
  parsed.pathname = pathname.endsWith('/v1')
    ? pathname.slice(0, -3) || '/'
    : pathname || '/'
  parsed.search = ''
...
export function getLlamaCppChatBaseUrl(baseUrl?: string): string {
  return `${getLlamaCppApiBaseUrl(baseUrl)}/v1`
}

export async function hasLocalLlamaCpp(baseUrl?: string): Promise<boolean> {
  const { signal, clear } = withTimeoutSignal(1200)
  try {
    const response = await fetch(`${getLlamaCppChatBaseUrl(baseUrl)}/models`, {
      method: 'GET',
      signal,
    })
    return response.ok
...
export async function listLlamaCppModels(baseUrl?: string): Promise<string[]> {
  const { signal, clear } = withTimeoutSignal(5000)
  try {
    const response = await fetch(`${getLlamaCppChatBaseUrl(baseUrl)}/models`, {
      method: 'GET',
