...
/**
 * If Windows, set the SHELL environment variable.
 * Prefers git-bash if available; falls back to PowerShell (pwsh or powershell).
 * Does NOT exit the process if neither is found — certain flows (e.g. API mode) don't need a shell.
 */
export function setShellIfWindows(): void {
  if (getPlatform() === 'windows') {
    const shellPath = findWindowsShellPath()
    if (shellPath) {
      process.env.SHELL = shellPath
      logForDebugging(`Using shell path: "${shellPath}"`)
    } else {
      logForDebugging('No shell found (git-bash, pwsh, or powershell). Shell commands may fail.')
    }
  }
}

/**
 * Find the best available Windows shell: git-bash > pwsh > powershell.exe.
 * Returns null if none found (instead of exiting the process).
 */
export const findWindowsShellPath = memoize((): string | null => {
  // 1. Try git-bash (best bash compatibility)
  const gitBash = tryFindGitBashPath()
  if (gitBash) return gitBash

  // 2. Try pwsh (PowerShell Core 7+)
  const pwsh = findExecutable('pwsh')
  if (pwsh) return pwsh

  // 3. Try classic powershell 5.1
  const ps51 = findExecutable('powershell')
  if (ps51) return ps51
...
/**
 * Try to find git-bash bash.exe; returns null instead of exiting.
 */
function tryFindGitBashPath(): string | null {
  if (process.env.CLAUDE_CODE_GIT_BASH_PATH) {
    ...
    logForDebugging(`CLAUDE_CODE_GIT_BASH_PATH not found: "${process.env.CLAUDE_CODE_GIT_BASH_PATH}"`)
    return null
  }
 ...
 * Falls back to PowerShell if git-bash is not installed.
 * @deprecated Prefer findWindowsShellPath() which gracefully falls back to PowerShell.
 */
export const findGitBashPath = memoize((): string => {
  const gitBash = tryFindGitBashPath()
  if (gitBash) return gitBash

  // Fall back to PowerShell so the process doesn't exit
  const ps = findExecutable('pwsh') ?? findExecutable('powershell')
  if (ps) {
    logForDebugging(`git-bash not found; using PowerShell as shell: "${ps}"`)
    return ps
  }

  // biome-ignore lint/suspicious/noConsole:: intentional console output
  console.error(
    'OpenClaude on Windows requires either git-bash (https://git-scm.com/downloads/win) or PowerShell (pwsh / powershell). ' +
    'If git-bash is installed but not in PATH, set CLAUDE_CODE_GIT_BASH_PATH=C:\\Program Files\\Git\\bin\\bash.exe',
  )
