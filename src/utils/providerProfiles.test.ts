...
describe('getProviderPresetDefaults', () => {
  test('llamacpp preset defaults to local llama-server', async () => {
    delete process.env.OPENAI_MODEL
    delete process.env.OPENAI_BASE_URL
    const { getProviderPresetDefaults } = await importFreshProviderModules()

    const defaults = getProviderPresetDefaults('llamacpp')

    expect(defaults.baseUrl).toBe('http://localhost:8080/v1')
