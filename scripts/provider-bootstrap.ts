...
function parseProviderArg(): ProviderProfile | 'auto' {
  ...
  if (p === 'llamacpp' || p === 'openai' || p === 'ollama' || p === 'codex' || p === 'gemini' || p === 'atomic-chat') return p as ProviderProfile
  return 'auto'
}
...
async function main(): Promise<void> {
  ...
  if (provider === 'auto') {
    if (await hasLocalLlamaCpp(argBaseUrl || undefined)) {
  ...
  if (selected === 'llamacpp') {
    // Discover which model is currently loaded in llama-server
    const model = argModel || (await listLlamaCppModels(argBaseUrl || undefined))[0] || 'local-model'
    env = buildLlamaCppProfileEnv({
      baseUrl: argBaseUrl,
      model,
    })
    ...
    if (!builtEnv) {
      ...
      if (!credentials.apiKey) {
        console.error('Codex profile requires CODEX_API_KEY.')
      } else {
  ...
  console.log('Next: bun run dev:profile  (or just run start.bat)')
}
