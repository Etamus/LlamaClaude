...
function parseLaunchOptions(argv: string[]): LaunchOptions {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    if (
      (lower === 'auto' || lower === 'llamacpp' || lower === 'openai' ||
       lower === 'ollama' || lower === 'codex' || lower === 'gemini' ||
       lower === 'atomic-chat') &&
...
async function main(): Promise<void> {
  ...
  if (!requestedProfile) {
    console.error('Usage: bun run scripts/provider-launch.ts [llamacpp|openai|ollama|codex|gemini|atomic-chat|auto] [--fast] [--goal <latency|balanced|coding>] [-- <cli args>]')
    process.exit(1)
  }
  ...
  if (requestedProfile === 'auto') {
    ...
    profile = requestedProfile as ProviderProfile
  }

  if (profile === 'llamacpp') {
    if (!resolvedLlamaCppModel && (!persisted?.env?.OPENAI_MODEL)) {
  ...
  if (
    ...
    if (!resolvedOllamaModel) {
      console.error('No viable Ollama chat model was discovered. Pull a model first or use llamacpp instead.')
      process.exit(1)
    }
  }
  ...
  if (
    ...
    if (!resolvedAtomicChatModel) {
      console.error('Atomic Chat is running but no model is loaded.')
      process.exit(1)
    }
  }
  ...
  // For llamacpp, ensure the base URL and model are set
  ...
  if (profile === 'gemini' && !env.GEMINI_API_KEY) {
    console.error('GEMINI_API_KEY is required for gemini profile.')
    process.exit(1)
  }

  if (profile === 'openai' && (!env.OPENAI_API_KEY || env.OPENAI_API_KEY === 'SUA_CHAVE')) {
    console.error('OPENAI_API_KEY is required for openai profile and cannot be SUA_CHAVE.')
    process.exit(1)
  }
  ...
  if (profile === 'codex') {
    ...
    if (!credentials.apiKey) {
      console.error('CODEX_API_KEY is required for codex profile.')
      process.exit(1)
    }
    if (!credentials.accountId) {
      console.error('CHATGPT_ACCOUNT_ID is required for codex profile.')
      process.exit(1)
    }
  }
